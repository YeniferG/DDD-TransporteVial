...
public class CrearServicioConductorElegidoUseCase extends UseCase<TriggeredEvent<CotizacionAgregada>, ResponseEvents> {
    ...
    public void executeUseCase(TriggeredEvent<CotizacionAgregada> triggeredEvent) {
        ...
        if((event.getDireccionInicial().value().municipio().value().isBlank() || event.getDireccionDestino().value().municipio().value().isBlank())
            && (event.getDireccionInicial().value().direccion().isBlank() || event.getDireccionDestino().value().direccion().isBlank())){
